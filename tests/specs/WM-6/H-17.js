...
module.exports = {
  'H-17 test 0num': function (client) {
    auth
        .login(client);
    ...
    var dt = utils.date_add(365);
    utils.open_mng_start_date(client, dt)
  ...
  'H-17 test 1num': function (client) {
    auth
        .login(client);
...
//        .expect.element('#listSearchResultId > tbody').to.be.present
    client
        .source( function(result){
            ...
            this.assert.equal(2, $('#listSearchResultId > tbody').children().length );
        })
/*
    auth.logout(client); // logout
    client.end();
*/
  ...
  'H-17 test 21num': function (client) {
    auth
        .login(client);
//    utils.opendir1(client) // test対象Directoryを開く
    utils.open_mng(client)
    ...
    client
        .expect.element('#listSearchResultId > tbody').to.not.be.present
    utils.open_proc_select_span(client)
    utils.open_mng_start_date(client, "2016/09/20")
    utils.open_mng_expired_date(client, "2016/09/28")
    utils.click_dialog_search(client)
    client
        .waitForElementNotPresent('.progress-image > img', 16000)
    ...
    client
        .source( function(result){
            const $ = cheerio.load(result.value);
            this.assert.equal(21, ($('#listSearchResultId > tbody').children().length - 1) );
